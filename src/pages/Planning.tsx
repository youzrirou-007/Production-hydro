...
import { PlanningMinageTable } from '../components/planning/PlanningMinageTable';
import { PlanningDeblayageTable } from '../components/planning/PlanningDeblayageTable';
import { PlanningExtractionTable } from '../components/planning/PlanningExtractionTable';
import { PlanningMaintenanceTable } from '../components/planning/PlanningMaintenanceTable';
import { PlanningActivityTabs } from '../components/planning/PlanningActivityTabs';
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="w-full space-y-4 bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
            ...
            {/* Sheet Tabs */}
            <PlanningActivityTabs
              active={activeSheetTab}
              onChange={(tab) => setActiveSheetTab(tab)}
            />

            <fieldset disabled={isLockedByNiveau2 || isMonthClosedForPlanning} className="space-y-4">
              ...
              <PlanningMinageTable
                ...
                deleteMinageRowAt={deleteMinageRowAt}
                isMinageRowRemovable={isMinageRowRemovable}
                addRowToMinageSector={addRowToMinageSector}
                employees={employees}
                chantiers={chantiers}
                getSectorBadgeStyles={getSectorBadgeStyles}
                makeExcelKeyHandler={makeExcelKeyHandler}
                getEmployeeName={getEmployeeName}
                sectorChiefs={sectorChiefs}
                setSectorChiefs={setSectorChiefs}
                sectorBoutefeus={sectorBoutefeus}
                setSectorBoutefeus={setSectorBoutefeus}
                handleToggleManualOverride={handleToggleManualOverride}
                isSectorMatching={isSectorMatching}
              />
            )}            {/* SHEET 2: LOADER DEBLAYAGE INTERACTIVE EXCEL GRID (3 SHIFTS VERTICALLY STACKED) */}
            {activeSheetTab === 'deblayage' && (
              <PlanningDeblayageTable
                deblayageRowsByPost={deblayageRowsByPost}
                updateDeblayageCell={updateDeblayageCell}
                addRowToDeblayageSector={addRowToDeblayageSector}
                addStockRowToDeblayageSector={addStockRowToDeblayageSector}
                deleteDeblayageRowAt={deleteDeblayageRowAt}
                isDeblayageRowRemovable={isDeblayageRowRemovable}
                employees={employees}
                chantiers={chantiers}
                getSectorBadgeStyles={getSectorBadgeStyles}
                makeExcelKeyHandler={makeExcelKeyHandler}
                platformSettings={platformSettings}
                getBucketCapacity={getBucketCapacity}
                isSectorMatching={isSectorMatching}
              />
            )}
            ...
            {activeSheetTab === 'maintenance' && (
              <PlanningMaintenanceTable
                maintenanceRowsByPost={maintenanceRowsByPost}
                updateMaintenanceCell={updateMaintenanceCell}
                addRowToMaintenance={addRowToMaintenance}
                deleteMaintenanceRowAt={deleteMaintenanceRowAt}
                employees={employees}
                getEmployeeName={getEmployeeName}
                platformSettings={platformSettings}
              />
